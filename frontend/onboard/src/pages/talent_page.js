...
class TalentPage extends React.Component {
	...
	updateSelections = (name) => {
		...
		if (arr[name]) {
			...
			filtered = this.state.talent.filter((talent) => {
				return talent.job_category === this.state.categoryToName[name];
			});
			let currFilter = [];
			if (!map.has(name)) {
	...
	componentDidMount() {
		axios
			.get('http://localhost:5001/api/workexp')
			.then((res) => {
				this.setState({ workExp: res.data.data });
				this.retrieveTalentList().then((res) => {
		...
		return (
			<div className="talent-main-container">
				<div className="talent-top-bar">
					<text className="talent-logo">Layup</text>
				</div>
				<div className="talent-container">
					<div className="talent-job-field-container">
						<text className="talent-job-field-text">Job Field</text>
						<button
							className={
								this.state.interests['business']
						...
						<button
							className={
								this.state.interests['customerSupport']
									? 'talent-button-clicked'
									: 'talent-button'
							}
						...
						<button
							className={
								this.state.interests['dataScience']
									? 'talent-button-clicked'
									: 'talent-button'
							}
						...
						<button
							className={
								this.state.interests['engineering']
									? 'talent-button-clicked'
									: 'talent-button'
							}
						...
						<button
							className={
								this.state.interests['financeAccounting']
									? 'talent-button-clicked'
									: 'talent-button'
							}
						...
						<button
							className={
								this.state.interests['informationTechnology']
									? 'talent-button-clicked'
									: 'talent-button'
							}
						...
						<button
							className={
								this.state.interests['legalPolicy']
									? 'talent-button-clicked'
									: 'talent-button'
							}
						...
						<button
							className={
								this.state.interests['marketing']
									? 'talent-button-clicked'
									: 'talent-button'
							}
						...
						<button
							className={
								this.state.interests['productManagement']
									? 'talent-button-clicked'
									: 'talent-button'
							}
						...
						<button
							className={
								this.state.interests['researchScience']
									? 'talent-button-clicked'
									: 'talent-button'
							}
					...
					<div className="talent-results">
						...
						{this.returnTalentList().map((talent) => {
							return talent;
						})}
					</div>
				</div>
