...
function ResumeUpload() {
	...
	const uploadResume = () => {
		const filename =
			location.state.firstName + '_' + location.state.lastName + '_Resume.pdf';
		...
		s3.upload(params, (err, data) => {
			if (err) {
				...
				axios
					.post('http://localhost:5001/talent-entry', location.state)
					.then((res, err) => {
						if (err) {
							console.log(err);
						} else {
		...
		<div className="centered-container">
			...
			<div className="Row-upload-resume">
				<label className="file-upload">
					...
					<i className="fa fa-cloud-upload" style={{ fontSize: 22 }} />
					<span className="file-upload-label">
						{uploadedFile ? file.name : 'Attach Resume'}
					</span>
				</label>
			</div>
