...
function SignUp() {
	...
	useEffect(() => {
		...
		if (keyParam.get('_key') !== null) {
			...
			axios
				.post('http://localhost:5001/api/signup/confirm', {
				...
				.then((res) => {
					...
					if (res.data.success) {
						...
						setFirstSignup(
							!res.data.data.finished_sign_up && !res.data.data.key_used
						);
					}
				})
				.catch((err) => {
	...
	const invaidSignUp = () => {
		return (
			<div
				className="centered-container-welcome" /*style = {{justifyContent:"space-around"}}*/
			>
				...
				<div className="endorse-message">
					...
					<div className="endorse-message">
						Please contact the person who gave you this key for more
						information.
					</div>
					<div className="endorse-message">
	...
	const newSignUpValid = () => {
		return (
			<div className="centered-container">
				<div className="signup-logo">Layup</div>
				{firstSignup ? (
					<>
						<div className="contact-title-text">
							Welcome {firstName},{' '}
							<span className="underline-company">{company}</span> has endorsed
							you
						</div>
				...
				<div className="contact-info-container">
					...
					{matchingPasswords ? null : (
						<div className="Row-upload">
							<p className="wrong-password-text">Passwords do not match!</p>
						</div>
					)}
					{validEmail ? null : (
						<div className="Row-upload">
							<p className="wrong-password-text">Invalid Email</p>
						</div>
					)}
					{!allFieldsFilledOut && clickedSignUp ? (
						<div className="Row-upload">
							<p className="wrong-password-text">Fill out all fields!</p>
						</div>
					) : null}
	...
	return validKey ? newSignUpValid() : invaidSignUp();
}
